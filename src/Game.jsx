...
// Likes disponibles (no escrow) para un player
function availableLikes(player) {
  return (player?.likes || 0) - (player?.likesEscrow || 0);
}
...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  // v20: chequeo periódico de expiración de ofertas (cada 30s) y al montar
  useEffect(() => {
    let cancelled = false;
    const doCheck = async () => {
      if (cancelled) return;
      try {
        ...
        for (const [offerId, o] of Object.entries(offers)) {
          ...
          if (o.expiresAt && o.expiresAt <= now) {
            // Solo el owner o el buyer ejecutan la expiración (evita que todos los clientes compitan).
            if (o.ownerId === pid || o.buyerId === pid) {
              await expireOfferById(offerId).catch(e => console.error("expireOffer err", e));
            }
          }
        }
      } catch (e) { console.error("offer expiration check failed", e); }
    };
    doCheck();
  ...
  const toggleLike = async (mObj) => {
    ...
    if (me.likedObjectId === mObj.uid) {
      ...
      // Si el contexto actual es la sub-tab "Me gusta" del perfil, o un feed fullscreen
      // abierto desde ahí, hay que pedir confirmación porque el item desaparecerá de la vista.
      const inLikedSubTab = tab === "profile" && walletSubTab === "liked" && !feedView;
      const inLikedFeedView = feedView && feedView.sourceType === "liked";
      if (inLikedSubTab || inLikedFeedView) {
  ...
  const executeSale = async (mObj, buyerPid, sellerReceives, debtCancelled, offerId) => {
    ...
    // Snapshot común del ítem que usamos tanto para "obtained" (comprador)
    // como para "delivered" (vendedor). Incluimos metadata extra (fecha y
    // contraparte) usada por las sub-pestañas "Artículos recibidos" y
    // "Artículos entregados" del perfil.
    ...
    updates[`players/${mObj.ownerId}/delivered`] = [...(seller.delivered || []), {
      ...itemSnapshot,
      date: txDate,
      toId: buyerPid,
    }];
    ...
    Object.entries(players).forEach(([p, d]) => { if (d.likedObjectId === mObj.uid) updates[`players/${p}/likedObjectId`] = null; });
    updates[`market/${mObj.uid}`] = null;
    // v20: si venía de una oferta, descongelar el escrow del comprador y limpiar la offer
    if (offerId) {
      updates[`offers/${offerId}`] = null;
      const esc = (buyer.likesEscrow || 0) - likeCount;
      updates[`players/${buyerPid}/likesEscrow`] = esc < 0 ? 0 : esc;
    }
    await update(ref(db, "game"), updates);
    addLog("sell", `💰 ${seller.name} → ${buyer.name}: "${mObj.name}" (${sr} ${t.likes})`, [mObj.ownerId, buyerPid]);
    if (buyerNew < 0 && dc === 0) addLog("credit", `🔴 ${buyer.name}: ${buyerNew} ${t.likes}`, [buyerPid]);
  ...
  const makeBuyOffer = async (mObj) => {
    ...
    // Chequeo 3: saldo disponible (considerando escrow)
    const avail = availableLikes(me);
    const debt = Math.abs(Math.min(0, me.likes || 0));
    let sr, dc;
    if ((me.likes || 0) >= 0) {
      if (avail < likeCount) return;
      sr = likeCount; dc = 0;
    } else {
    ...
    // Congelar likes en escrow (si el comprador no estaba en deuda)
    if ((me.likes || 0) >= 0) {
      updates[`players/${pid}/likesEscrow`] = (me.likesEscrow || 0) + likeCount;
    }
    await update(ref(db, "game"), updates);
  ...
  const acceptOffer = async (offerId) => {
    ...
    // Leer el item del market porque la offer solo tiene snapshot liviano
    ...
    }, o.buyerId, o.sellerReceives, o.debtCancelled, offerId);
  };
  ...
  const rejectOffer = async (offerId) => {
    ...
    // Descongelar escrow del buyer (solo si se había congelado; si el buyer estaba en deuda al ofertar, no había escrow)
    if ((buyerData.likes || 0) >= 0 || (buyerData.likesEscrow || 0) > 0) {
      const newEsc = Math.max(0, (buyerData.likesEscrow || 0) - price);
      updates[`players/${buyerId}/likesEscrow`] = newEsc;
    }
  ...
  const expireOfferById = async (offerId) => {
    ...
    updates[`market/${o.itemUid}/pendingOfferId`] = null;
    if ((buyerData.likes || 0) >= 0 || (buyerData.likesEscrow || 0) > 0) {
      updates[`players/${buyerId}/likesEscrow`] = Math.max(0, (buyerData.likesEscrow || 0) - price);
    }
    updates[`cooldowns/${buyerId}/${o.itemUid}`] = { until: Date.now() + COOLDOWN_MS };
    await update(ref(db, "game"), updates);
  ...
  const renderMarketCard = (mObj) => {
    ...
    const avail = availableLikes(me);
    const canBuy = isMarketItem && isLiked && !isOwn && !isLocked && likeCount > 0 && cdRemaining === 0 && ((me.likes || 0) >= 0 ? avail >= likeCount : likeCount >= debt);
    ...
    return (
      <div key={cardKey} style={{ marginBottom: "24px", borderBottom: `1px solid ${th.borderLight}`, paddingBottom: "16px" }}>
        ...
        <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div>
            <div style={{ fontSize: "14px", fontWeight: 600 }}>{ownerData?.name || me.name}</div>
    ...
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", color: th.text, display: "flex", flexDirection: "column" }} onClick={closeAllDropdowns}>
      ...
      <div style={{ flex: 1, paddingBottom: bottomH + 8 }}>
        ...
        {tab === "profile" && (() => {
          ...
          const myLikedItem = me.likedObjectId ? marketItems.find(mi => mi.uid === me.likedObjectId) : null;
            ...
            <div style={{ padding: "20px 16px", maxWidth: "600px", margin: "0 auto" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px" }}>
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: "16px", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name}</div>
                  <div style={{ fontSize: "12px", color: th.textMuted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</div>
                </div>
                {/* Hamburger menu */}
              ...
              <div style={{
                ...
                {[
                  ...
                  { id: "liked", label: t.walletLiked, count: myLikedItem ? 1 : 0 },
                ].map(st => (
                  <button
                    key={st.id}
              ...
              {walletSubTab === "liked" && (
                <>
                  {!myLikedItem ? (
                    <p style={{ color: th.textMuted, fontSize: "13px" }}>{t.nothingLiked}</p>
                  ) : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {(() => {
                        const imgs = getItemImages(myLikedItem);
                        const likeCount = getLikes(myLikedItem.uid);
                        return (
                          <div
                            key={myLikedItem.uid}
                            onClick={() => openFeedView(t.walletLiked, "liked", `m:${myLikedItem.uid}`)}
                            style={{ background: th.bgCard, borderRadius: "10px", overflow: "hidden", border: `1px solid ${th.borderLight}`, cursor: "pointer" }}
                          >
                            ...
                            <div style={{ padding: "6px" }}>
                              <div style={{ fontSize: "11px", fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{myLikedItem.name}</div>
                            </div>
                          </div>
                        );
                      })()}
        ...
        return (
          <div style={{
            ...
            <p style={{ fontSize: "12px", color: th.textSecondary, marginBottom: 10, lineHeight: 1.4 }}>
              <strong style={{ color: th.text }}>{buyer.name || "?"}</strong> {t.wantsToBuy} <strong style={{ color: th.text }}>"{incomingOffer.itemSnapshot?.name || ""}"</strong>.
      ...
      {feedView && (() => {
        ...
        const myLikedItemLive = me.likedObjectId ? marketItems.find(mi => mi.uid === me.likedObjectId) : null;
