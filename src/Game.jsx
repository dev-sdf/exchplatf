...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  const [likeRemovingAnim, setLikeRemovingAnim] = useState(false);
  ...
  const myLikedItem = me.likedObjectId
    ? (marketItems.find(mi => mi.uid === me.likedObjectId) ||
       (myBuyerPending?.itemUid === me.likedObjectId
         ...
         : null))
    : null;
  // Reset animation flag when like is actually cleared
  useEffect(() => { if (!myLikedItem) setLikeRemovingAnim(false); }, [!!myLikedItem]);
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  >
                    <div style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 0, flex: 1 }}>
                      ...
                      {myLikedItem && (
                        <div style={{ fontSize: 10, color: th.text, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{myLikedItem.name}</div>
                      )}
                    </div>
                    <div style={{ width: 72, height: 72, borderRadius: 8, overflow: "hidden", flexShrink: 0, border: `2px solid ${likeAccentBorder}` }}>
                      {myLikedItem ? (
                        <div style={{
                          width: "100%", height: "100%", boxShadow: `0 0 6px ${likeAccent}66`,
                          animation: likeRemovingAnim ? "likeRemove 0.4s ease-in forwards" : "none",
                        }}>
