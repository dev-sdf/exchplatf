...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  const [feedView, setFeedView] = useState(null); // { title, items, startIdx } | null
  ...
  const renderMarketCard = (mObj) => {
    // mObj puede venir del market o de me.withdrawn/obtained (en esos casos no tiene uid ni ownerId)
    const isMarketItem = !!mObj.uid && !!mObj.ownerId;
    const likeCount = isMarketItem ? getLikes(mObj.uid) : 0;
    const isLiked = isMarketItem && me.likedObjectId === mObj.uid;
    const isOwn = isMarketItem && mObj.ownerId === pid;
    ...
    const cardKey = mObj.uid || `${mObj.name}-${mObj.createdAt || 0}`;
  ...
  const openFeedView = (title, items, startIdx) => {
    setFeedView({ title, items, startIdx });
  };
      ...
      <div style={{ flex: 1, paddingBottom: bottomH + 8 }}>
        ...
        {tab === "profile" && (() => {
          ...
          const myActiveFeed = [
            ...
            ...myWithdrawn.map(w => ({ ...w, _sourceTag: "withdrawn" })),
          ];
          return (
            <div style={{ padding: "20px 16px", maxWidth: "600px", margin: "0 auto" }}>
                  ...
                  {myActiveFeed.length === 0 ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {myActiveFeed.map((item, idx) => {
                        ...
                        const withdrawnIdx = !isExhibited ? myWithdrawn.indexOf(item) : -1;
                        return (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {(() => {
                        ...
                        return (
                          <div
                            key={myLikedItem.uid}
                            onClick={() => openFeedView(t.walletLiked, [myLikedItem], 0)}
                            style={{ background: th.bgCard, borderRadius: "10px", overflow: "hidden", border: `1px solid ${th.borderLight}`, cursor: "pointer" }}
                          >
