...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  const [profileStripH, setProfileStripH] = useState(0);
  ...
  const profileStripRef = useRef(null);
    ...
    const unsub = onValue(ref(db, `game/notifications/${pid}`), (snap) => {
      ...
      Object.entries(data).forEach(([nid, n]) => {
        ...
        if (n.time && n.time >= mountTimeRef.current) {
          // Crear toast
          setToasts([{ id: nid, type: n.type, params: n.params || {}, createdAt: Date.now() }]);
        }
      });
    });
    return () => unsub();
  ...
  }, [toasts[0]?.id]); // se re-ejecuta cada vez que cambia el toast activo

  // Medir altura del strip de perfil para que el search bar sepa dónde pegarse
  useEffect(() => {
    if (!profileStripRef.current) return;
    const obs = new ResizeObserver(entries => {
      setProfileStripH(entries[0].contentRect.height);
    });
    obs.observe(profileStripRef.current);
  ...
  // Like activo del usuario — visible siempre (strip de perfil + feedView)
  ...
  const toggleLike = async (mObj) => {
    ...
    if (me.likedObjectId === mObj.uid) {
      // Siempre pedir confirmación antes de quitar el like.
      setUnlikeConfirmModal({ item: mObj });
      return;
    } else {
      const wasMoving = !!me.likedObjectId; // había un like previo en otro artículo
      ...
      if (wasMoving) {
        setToasts([{ id: `likemoved-${Date.now()}`, type: "likeMoved", params: { item: mObj.name }, createdAt: Date.now(), bottom: true }]);
      }
    }
  };
  ...
  const makeBuyOffer = async (mObj) => {
    ...
    if (getItemOffer(mObj.uid)) {
      setToasts([{ id: `err-${Date.now()}`, type: "offerHasActive", params: {}, createdAt: Date.now() }]);
      return;
    }
    // Chequeo 2: cooldown
  ...
  const renderMarketCard = (mObj) => {
    ...
    const cardLikeAccent = isLiked ? (walletLocked ? "#f59e0b" : "#f43f5e") : null;
    return (
      <div key={cardKey} style={{
        position: "relative",
        marginBottom: "24px", paddingBottom: "16px",
        borderBottom: isLiked ? `1px solid ${cardLikeAccent}55` : `1px solid ${th.borderLight}`,
        borderRadius: isLiked ? "10px" : 0,
        boxShadow: isLiked ? `0 0 14px ${cardLikeAccent}44` : "none",
      }}>
        {isLiked && (
          <div style={{
            position: "absolute", inset: 0, borderRadius: "10px",
            border: `3px solid ${cardLikeAccent}`,
        ...
        <div style={{ padding: "10px 14px 0" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "6px" }}>
            <span onClick={() => { if (isMarketItem && !isLocked && !isOwn) toggleLike(mObj); }} style={{ cursor: isMarketItem && !isOwn && !isLocked ? "pointer" : "default", display: "flex", alignItems: "center" }}>
              {isLiked ? (
                <svg width="26" height="26" viewBox="0 0 24 24" fill="#ef4444" stroke="#ef4444" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            {isOwn ? (
              ...
              <span style={{ fontSize: "14px", fontWeight: 700 }}>{likeCount}</span>
            )}
            {isLocked && <span style={{ fontSize: "11px", color: "#f59e0b", marginLeft: "auto" }}>🔒 {t.locked}</span>}
            {isMarketItem && !isOwn && !isLocked && cdRemaining === 0 && (
              <span style={{ fontSize: "12px", color: isLiked ? th.textMuted : th.textMuted, marginLeft: "auto" }}>
                {isLiked ? t.tapToUnlike : t.tapToLike}
              </span>
            )}
          </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", color: th.text, display: "flex", flexDirection: "column" }} onClick={closeAllDropdowns}>
      ...
      <div style={{
        ...
        {/* IZQUIERDA: barra de búsqueda (solo en mercado) */}
        {tab === "market" ? (
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "5px", minWidth: 0 }} onClick={e => e.stopPropagation()}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <div onClick={() => setSearchModeOpen(!searchModeOpen)} style={{ background: th.bgInput, border: `1px solid ${th.borderColor}`, borderRadius: "8px", padding: "5px 8px", fontSize: "11px", color: th.text, cursor: "pointer", whiteSpace: "nowrap" }}>
                {searchModeLabels[searchMode]}
              </div>
              {searchModeOpen && (
                <div style={{ position: "absolute", top: "100%", left: 0, background: th.dropdown, border: `1px solid ${th.dropdownBorder}`, borderRadius: "8px", marginTop: "4px", zIndex: 70, boxShadow: "0 8px 24px rgba(0,0,0,0.3)", minWidth: "110px" }}>
            ...
            {searchMode === "categories" ? (
              <div style={{ flex: 1, position: "relative", minWidth: 0 }}>
                <svg style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", opacity: 0.45, color: th.text }} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <div onClick={() => setCatDropdownOpen(!catDropdownOpen)} style={{ background: th.bgInput, border: `1px solid ${th.borderColor}`, borderRadius: "8px", padding: "5px 8px 5px 26px", fontSize: "11px", color: selectedCategoryIdx.length ? th.text : th.textMuted, cursor: "pointer", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {selectedCategoryIdx.length === 0 ? t.allCategories : selectedCategoryIdx.map(i => cats[i]).join(", ")}
                </div>
                {catDropdownOpen && (
                  <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: th.dropdown, border: `1px solid ${th.dropdownBorder}`, borderRadius: "8px", marginTop: "4px", zIndex: 70, maxHeight: "280px", overflow: "auto", boxShadow: "0 8px 24px rgba(0,0,0,0.3)" }}>
              ...
              <div style={{ flex: 1, position: "relative", minWidth: 0 }}>
                ...
                <input type="text" value={searchText} onChange={e => setSearchText(e.target.value)}
                  ...
                  style={{ width: "100%", background: th.bgInput, border: `1px solid ${th.borderColor}`, borderRadius: "8px", padding: "5px 8px 5px 26px", fontSize: "11px", color: th.text, outline: "none" }} />
              </div>
            )}
          </div>
        ) : <div style={{ flex: 1 }} />}
        ...
        <div style={{ display: "flex", gap: "6px", alignItems: "center", flexShrink: 0 }}>
          ...
          <div ref={profileMenuRef} style={{ position: "relative" }} onClick={e => e.stopPropagation()}>
            <button
            ...
            >
              {me.photo ? (
                <img src={me.photo} style={{ width: 30, height: 30, borderRadius: "50%", display: "block", border: `2px solid ${th.borderColor}` }} />
              ) : (
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: th.bgCard, border: `2px solid ${th.borderColor}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: th.textSecondary }}>
            ...
            {profileMenuOpen && (
              <div style={{
                ...
                background: th.dropdown, border: `1px solid ${th.dropdownBorder}`,
                borderRadius: "14px", boxShadow: "0 12px 32px rgba(0,0,0,0.4)",
                zIndex: 150, overflow: "hidden",
              }}>
                {/* Email */}
                <div style={{ padding: "12px 16px 10px", borderBottom: `1px solid ${th.borderLight}` }}>
                  <div style={{ fontSize: "12px", color: th.textMuted, textAlign: "center", wordBreak: "break-all" }}>{user.email}</div>
                </div>
                {/* Foto + nombre + rating */}
                <div style={{ padding: "16px 16px 14px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, borderBottom: `1px solid ${th.borderLight}` }}>
                  <div
                    onClick={() => { setProfileMenuOpen(false); setProfileEditOpen(true); }}
                    style={{ position: "relative", width: 72, height: 72, cursor: "pointer" }}
                    title={t.profileChangePhotoTooltip}
                  >
                    {me.photo ? (
                      <img src={me.photo} style={{ width: 72, height: 72, borderRadius: "50%", display: "block" }} />
                    ) : (
                      <div style={{ width: 72, height: 72, borderRadius: "50%", background: th.bgCard, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700, color: th.textSecondary }}>
                        {(me.name || "?")[0].toUpperCase()}
                      </div>
                    )}
                    <div style={{
                      ...
                      border: `2px solid ${th.dropdown}`,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 11, fontWeight: 800,
                    }}>✎</div>
                  </div>
                  <div style={{ fontSize: "15px", fontWeight: 700, color: th.text, textAlign: "center" }}>{me.name}</div>
                  <div style={{ fontSize: "12px", color: th.textMuted }}>{getRatingStr(me)}</div>
                </div>
                {/* Modalidad nocturna + Idioma */}
                <div style={{ borderBottom: `1px solid ${th.borderLight}` }}>
                  <div style={{ padding: "11px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ fontSize: 13, color: th.text }}>{t.nightMode}</span>
                    <div style={{ display: "flex", gap: 4 }}>
                      {["dark", "light"].map(mode => (
                        <button key={mode}
                          onClick={(e) => { e.stopPropagation(); setTheme(mode); }}
                          style={{
                            padding: "4px 9px", fontSize: 11, fontWeight: theme === mode ? 700 : 400,
                            borderRadius: 6, border: `1px solid ${theme === mode ? "#38bdf8" : th.borderColor}`,
                            background: theme === mode ? "rgba(56,189,248,0.15)" : th.bgInput,
                            color: theme === mode ? "#38bdf8" : th.text, cursor: "pointer",
                          }}
                        >{mode === "dark" ? "ON" : "OFF"}</button>
                      ))}
                    </div>
                  </div>
                  <div style={{ padding: "11px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, borderTop: `1px solid ${th.borderLight}` }}>
                    <span style={{ fontSize: 13, color: th.text }}>{t.language}</span>
                    <div style={{ display: "flex", gap: 4 }}>
                      {Object.entries(LANGUAGE_NAMES).map(([code]) => (
                        <button key={code}
                          onClick={(e) => { e.stopPropagation(); setLang(code); }}
                          style={{
                            padding: "4px 9px", fontSize: 11, fontWeight: code === lang ? 700 : 400,
                            borderRadius: 6, border: `1px solid ${code === lang ? "#38bdf8" : th.borderColor}`,
                            background: code === lang ? "rgba(56,189,248,0.15)" : th.bgInput,
                            color: code === lang ? "#38bdf8" : th.text, cursor: "pointer",
                          }}
                ...
                <div
                  ...
                  style={{ padding: "12px 16px", fontSize: 13, color: th.text, cursor: "pointer", borderBottom: `1px solid ${th.borderLight}` }}
                >
                  {t.myActivity}
                </div>
                <div
                  onClick={() => { setProfileMenuOpen(false); onLogout(); }}
                  style={{ padding: "12px 16px", fontSize: 13, color: "#ef4444", cursor: "pointer", fontWeight: 600 }}
                >
                  {t.logout}
                </div>
              </div>
      ...
      {/* STRIP DE BILLETERA - visible solo en el tab Mercado */}
      {tab === "market" && <div
        ref={profileStripRef}
        style={{
      ...
      >
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "8px 14px" }}>
          {/* Contenedor agrupador: Billetera + Like activo */}
          {(() => {
            const likeAccent = walletLocked ? "#f59e0b" : "#f43f5e";
            const likeAccentBg = walletLocked ? "rgba(245,158,11,0.08)" : "rgba(244,63,94,0.06)";
            const likeAccentBorder = walletLocked ? "rgba(245,158,11,0.35)" : "rgba(244,63,94,0.3)";
            return (
              <div style={{
                borderRadius: "12px", padding: "10px 12px",
                background: likeAccentBg,
                border: `1px solid ${likeAccentBorder}`,
                display: "flex", flexDirection: "column", gap: 8,
                transition: "background 0.3s, border-color 0.3s",
              }}>
                {/* Fila: Billetera + Like activo */}
                <div style={{ display: "flex", gap: "10px" }}>
                  {/* Billetera */}
                  <div style={{
                    flex: 1, background: th.bgCard, borderRadius: "8px", padding: "8px 10px",
                    border: `1px solid ${th.borderLight}`,
                    display: "flex", flexDirection: "column", gap: 3, minWidth: 0,
                  }}>
                    <div style={{ fontSize: "9px", color: th.textSecondary, textTransform: "uppercase", letterSpacing: 0.3, fontWeight: 700 }}>{t.walletTitle}</div>
                    <div style={{ fontSize: "22px", fontWeight: 800, color: (me.likes || 0) >= 0 ? "#10b981" : "#ef4444", lineHeight: 1.1 }}>{me.likes || 0} ❤️</div>
                  </div>
                  {/* Like activo */}
                  <div
                    onClick={myLikedItem ? () => {
                    ...
                    style={{
                      flexShrink: 0, background: th.bgCard, borderRadius: "8px", padding: "8px 10px",
                      border: `1px solid ${likeAccentBorder}`,
                      boxShadow: `0 0 0 1px ${likeAccent}22`,
                      display: "flex", flexDirection: "column", alignItems: "flex-start",
                      gap: 4,
                      cursor: myLikedItem ? "pointer" : "default",
                    }}
                  >
                      <div style={{ fontSize: "9px", color: likeAccent, textTransform: "uppercase", letterSpacing: 0.3, fontWeight: 700 }}>{t.likeStatus}</div>
                      <div style={{ position: "relative", width: 72, height: 72, borderRadius: 8, overflow: "hidden", flexShrink: 0, border: `2px solid ${likeAccentBorder}` }}>
                        {myLikedItem ? (
                          <>
                            <div style={{ width: "100%", height: "100%", boxShadow: `0 0 6px ${likeAccent}66` }}>
                              <ItemImage src={getItemImages(myLikedItem)[0]} theme={theme} t={t} />
                            </div>
                            {/* Corazón rojo en esquina inferior derecha */}
                            <div style={{ position: "absolute", bottom: 3, right: 3, lineHeight: 1, filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.6))" }}>
                          ...
                          <>
                            <div style={{ width: "100%", height: "100%", background: th.bgInput, display: "flex", alignItems: "center", justifyContent: "center", padding: 6 }}>
                              <div style={{ fontSize: 9, color: th.textMuted, textAlign: "center", lineHeight: 1.4 }}>{t.likeNotAssigned}</div>
                            </div>
                            {/* Corazón vacío en esquina inferior derecha */}
                            <div style={{ position: "absolute", bottom: 3, right: 3, lineHeight: 1, filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.4))" }}>
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: th.textMuted }}>
                                <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                              </svg>
                            </div>
                          </>
                ...
                {/* Banner de bloqueo */}
                {walletLocked && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(245,158,11,0.12)", borderRadius: 8, padding: "7px 10px" }}>
                    <span style={{ fontSize: 13, flexShrink: 0 }}>🔒</span>
                    <span style={{ fontSize: 10, color: "#f59e0b", fontWeight: 600, flex: 1, lineHeight: 1.4 }}>{t.walletLockedHint}</span>
                    <button
                      onClick={() => setAbortConfirmOpen(true)}
                      style={{
                        background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.35)",
                        borderRadius: 6, padding: "5px 9px", fontSize: 10, fontWeight: 700,
                        cursor: "pointer", color: "#ef4444", flexShrink: 0, whiteSpace: "nowrap",
                      }}
                    >{t.abortOffer}</button>
                  </div>
                )}
      ...
      <div style={{
        ...
        {toasts.filter(t => !t.bottom).map(toast => (
      ...
      {/* TOASTS — parte inferior */}
      <div style={{
        position: "fixed", bottom: bottomH + 12, left: "50%", transform: "translateX(-50%)",
        zIndex: 200, display: "flex", flexDirection: "column", gap: "8px",
        pointerEvents: "none", width: "min(92vw, 360px)",
      }}>
        {toasts.filter(t => t.bottom).map(toast => (
          <div key={toast.id} style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="#f43f5e" stroke="#f43f5e" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
              </svg>
              <span style={{ flex: 1, lineHeight: 1.4 }}>
                {(() => {
      ...
      {activityModalOpen && (() => {
        ...
        return (
          <div
            style={{ position: "fixed", inset: 0, background: th.overlayBg, zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}
            onClick={() => setActivityModalOpen(false)}
          >
            <div
              style={{
                background: th.modalBg, borderRadius: "16px", border: `1px solid ${th.borderColor}`,
                width: "100%", maxWidth: "460px", maxHeight: "75vh",
            ...
            >
              {/* Header */}
              <div style={{
                padding: "14px 16px", borderBottom: `1px solid ${th.borderLight}`,
                display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0,
              }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: th.text }}>{t.myActivity}</span>
                <button
                  onClick={() => setActivityModalOpen(false)}
                  style={{ background: "none", border: "none", cursor: "pointer", color: th.textMuted, fontSize: 18, lineHeight: 1, padding: "2px 6px" }}
                  aria-label={t.back}
                >✕</button>
              </div>
              {/* Content */}
              <div style={{ overflow: "auto", flex: 1, padding: "12px 14px" }}>
                {myEntries.length === 0 ? (
                  <p style={{ textAlign: "center", color: th.textMuted, fontSize: 13, padding: "32px 20px" }}>
                    {t.noActivity}
                  </p>
                ) : (
      ...
      {/* CONFIRMACIÓN DE ABORTAR */}
      {abortConfirmOpen && (
        <div
          style={{ position: "fixed", inset: 0, background: th.overlayBg, zIndex: 600, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}
          onClick={() => setAbortConfirmOpen(false)}
        >
          <div
            style={{ background: th.modalBg, borderRadius: "16px", border: `1px solid ${th.borderColor}`, width: "100%", maxWidth: "340px", padding: "24px 20px", display: "flex", flexDirection: "column", gap: 12 }}
            onClick={e => e.stopPropagation()}
          >
            <div style={{ fontSize: 15, fontWeight: 700, color: th.text }}>{t.abortConfirmTitle}</div>
            <div style={{ fontSize: 13, color: th.textSecondary, lineHeight: 1.5 }}>{t.abortConfirmMsg}</div>
            <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
              <button
                onClick={() => setAbortConfirmOpen(false)}
                style={{ flex: 1, background: th.bgCard, border: `1px solid ${th.borderColor}`, borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer", color: th.text }}
              >{t.cancel}</button>
              <button
                onClick={() => { setAbortConfirmOpen(false); abortTransaction(); }}
                style={{ flex: 1, background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.35)", borderRadius: 10, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer", color: "#ef4444" }}
              >{t.abortConfirmYes}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {feedView && (() => {
        ...
        if (feedView.sourceType === "active") {
          ...
          liveItems = myLikedItem ? [{ ...myLikedItem, _feedKey: `m:${myLikedItem.uid}` }] : [];
        }
