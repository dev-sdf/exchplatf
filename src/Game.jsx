...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  const [feedView, setFeedView] = useState(null); // { title, items, startIdx, source? } | null
  ...
  const publishProduct = async () => {
    ...
    try {
      if (editingItemUid) {
        ...
        // Subir solo las imágenes nuevas (las que tienen blob); las existentes se reusan
        const urls = await Promise.all(
          pubImages.map(async (imgObj, i) => {
            if (imgObj.existingUrl && !imgObj.blob) return imgObj.existingUrl;
            const imgRef = storageRef(storage, `products/${targetUid}-edit-${Date.now()}-${i}`);
  ...
  const toggleLike = async (mObj) => {
    ...
    if (me.likedObjectId === mObj.uid) {
      ...
      const inLikedFeedView = feedView && feedView.source === "liked";
  ...
  const rejectOffer = async () => {
    ...
    const ownerId = buyOffer.ownerId;
  ...
  const renderMarketCard = (mObj) => {
    ...
    const isLocked = isMarketItem && buyOffer && buyOffer.uid === mObj.uid;
    const debt = (me.likes || 0) < 0 ? Math.abs(me.likes) : 0;
    const canBuy = isMarketItem && isLiked && !isOwn && !isLocked && likeCount > 0 && ((me.likes || 0) >= 0 ? (me.likes || 0) >= likeCount : likeCount >= debt);
      ...
      <div key={cardKey} style={{ marginBottom: "24px", borderBottom: `1px solid ${th.borderLight}`, paddingBottom: "16px" }}>
        ...
        <div style={{ padding: "10px 14px 0" }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            ...
            {isMarketItem && !isOwn && !isLiked && !isLocked && <span style={{ fontSize: "12px", color: th.textMuted, alignSelf: "center" }}>{t.tapToLike}</span>}
            {isMarketItem && isLiked && !canBuy && !isOwn && <span style={{ fontSize: "12px", color: "#f59e0b", alignSelf: "center" }}>{t.yourLike}</span>}
          </div>
        </div>
      </div>
    );
  ...
  // Abrir un feed fullscreen con los items de una pestaña, empezando desde el clickeado
  const openFeedView = (title, items, startIdx, source) => {
    setFeedView({ title, items, startIdx, source });
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", color: th.text, display: "flex", flexDirection: "column" }} onClick={closeAllDropdowns}>
        ...
        {tab === "publish" && (
          <div style={{ padding: "20px 16px", maxWidth: "600px", margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
              <h2 style={{ fontSize: "18px", fontWeight: 700 }}>{editingItemUid ? t.editTitle : t.publishTitle}</h2>
              {editingItemUid && (
                <button
                  onClick={() => {
                    setEditingItemUid(null);
            ...
            <button onClick={publishProduct} disabled={publishing || !pubName.trim() || pubImages.length === 0}
              ...
              {publishing ? (editingItemUid ? t.saving : t.publishing) : (editingItemUid ? t.saveChanges : t.publish)}
            </button>
          </div>
        )}
                  ...
                  {myActiveFeed.length === 0 ? (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {myActiveFeed.map((item, idx) => {
                        ...
                        const isLocked = isExhibited && buyOffer && buyOffer.uid === item.uid;
                        ...
                        return (
                          <div key={isExhibited ? `m-${item.uid}` : `w-${idx}`} style={{ background: th.bgCard, borderRadius: "10px", overflow: "hidden", border: `1px solid ${th.borderLight}` }}>
                            <div
                              onClick={() => openFeedView(t.walletActive, myActiveFeed, idx)}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {myHistory.map((obj, i) => {
                        ...
                        return (
                          <div
                            key={i}
                            onClick={() => openFeedView(t.walletHistory, myHistory, i)}
                          ...
                          >
                            ...
                            <div style={{ padding: "6px" }}>
                              <div style={{ fontSize: "11px", fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{obj.name}</div>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))", gap: "10px" }}>
                      {(() => {
                        ...
                        return (
                          <div
                            key={myLikedItem.uid}
                            onClick={() => openFeedView(t.walletLiked, [myLikedItem], 0, "liked")}
                            style={{ background: th.bgCard, borderRadius: "10px", overflow: "hidden", border: `1px solid ${th.borderLight}`, cursor: "pointer" }}
                          >
      ...
      {/* WAITING */}
      {iMadeOffer && buyOffer && (
        <div style={{ position: "fixed", bottom: bottomH + 8, left: "50%", transform: "translateX(-50%)", background: th.modalBg, border: `1px solid ${th.borderColor}`, borderRadius: "12px", padding: "10px 20px", zIndex: 500, display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      {feedView && (
        <div style={{
          ...
          <div
            ref={(el) => {
              if (el && feedView.startIdx > 0) {
                // Scroll al item inicial (solo la primera vez)
                requestAnimationFrame(() => {
                  const target = el.querySelector(`[data-feed-idx="${feedView.startIdx}"]`);
          ...
          >
            <div style={{ maxWidth: 600, margin: "0 auto" }}>
              {feedView.items.map((item, i) => (
                <div key={i} data-feed-idx={i}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    if (feedView && feedView.source === "liked") {
                      setFeedView(null);
                    }
                  }}
