...
export default function Game({ user, onLogout, lang, setLang }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0e1a", fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#e2e8f0", display: "flex", flexDirection: "column" }}>
      ...
      {/* SEARCH BAR */}
      {tab === "market" && (
        <div style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,0.25)", flexShrink: 0 }}>
          <div style={{ maxWidth: "600px", margin: "0 auto", height: searchH, padding: "6px 12px", display: "flex", alignItems: "center", gap: "6px" }}>
          ...
          <div style={{ maxWidth: "600px", margin: "0 auto" }}>
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map(mObj => {
                ...
                return (
                  <div key={mObj.uid} style={{ marginBottom: "20px", borderBottom: "1px solid rgba(255,255,255,0.06)", paddingBottom: "16px" }}>
                    {/* Owner bar */}
                    <div style={{ padding: "10px 14px", display: "flex", alignItems: "center", gap: "8px" }}>
                      {ownerData?.photo ? <img src={ownerData.photo} style={{ width: 32, height: 32, borderRadius: "50%" }} /> :
                        <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px" }}>{(ownerData?.name || "?")[0]}</div>}
                      <div>
                        <div style={{ fontSize: "14px", fontWeight: 600 }}>{ownerData?.name} {isOwn && "(vos)"}</div>
                        <div style={{ fontSize: "11px", color: "#64748b" }}>{mObj.category}</div>
                      </div>
                    ...
                    <div onClick={() => { if (!isLocked && !isOwn) toggleLike(mObj); }}
                      style={{ width: "100%", aspectRatio: "1/1", cursor: isOwn || isLocked ? "default" : "pointer", overflow: "hidden", background: "#111" }}>
                    ...
                    {/* Actions bar - below image like Instagram */}
                    <div style={{ padding: "10px 14px 0" }}>
                      {/* Like button + count */}
                      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px" }}>
                        <span onClick={() => { if (!isLocked && !isOwn) toggleLike(mObj); }}
                          style={{ fontSize: "24px", cursor: isOwn || isLocked ? "default" : "pointer" }}>
                          {isLiked ? "❤️" : "🤍"}
                        </span>
                      ...
                      <div style={{ marginBottom: "8px" }}>
                        ...
                        {mObj.description && <span style={{ fontSize: "13px", color: "#94a3b8", marginLeft: "8px" }}>{mObj.description}</span>}
                      </div>

                      {/* Action buttons */}
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                        {/* Own object actions: always show if not locked */}
                        {isOwn && !isLocked && (
                          <>
                            <button onClick={() => openSellModal(mObj)} style={{
                              background: hasLikers ? "#10b981" : "rgba(255,255,255,0.08)",
                              color: hasLikers ? "#fff" : "#64748b", border: "none", borderRadius: "8px",
                              padding: "8px 20px", fontSize: "13px", fontWeight: 700, cursor: hasLikers ? "pointer" : "default",
                              opacity: hasLikers ? 1 : 0.5 }}>
                              {t.sell} {hasLikers ? `(${likeCount} ❤️)` : ""}
                            </button>
                            <button onClick={() => withdrawObject(mObj)} style={{
                              background: "rgba(255,255,255,0.08)", color: "#94a3b8", border: "none", borderRadius: "8px",
                              padding: "8px 20px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>
                              {t.withdraw}
                            </button>
                          </>
                        )}
                        {/* Buy offer for other's objects */}
                        {canBuy && (
                          <button onClick={() => makeBuyOffer(mObj)} style={{
                            background: canBuyIndebted ? "#f59e0b" : "#10b981",
                        ...
                        {!isOwn && !isLiked && !isLocked && (
                          <span style={{ fontSize: "12px", color: "#64748b", alignSelf: "center" }}>{t.tapToLike}</span>
                        )}
                        {isLiked && !canBuy && !isOwn && (
                          <span style={{ fontSize: "12px", color: "#f59e0b", alignSelf: "center" }}>{t.yourLike}</span>
                        )}
                      </div>
                    </div>
                  </div>
