...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  useEffect(() => {
    ...
    const doCheck = async () => {
      ...
      try {
        ...
        // Expirar pending transactions vencidas
        ...
        for (const [txId, ptx] of Object.entries(pendings)) {
          ...
          if (ptx.expiresAt && ptx.expiresAt <= now) {
            if (ptx.ownerId === pid || ptx.buyerId === pid) {
  ...
  const createPendingCore = async (mObj, buyerPid, sr, dc, price, offerIdOrNull) => {
    ...
    // Limpiar likedObjectId de TODOS los jugadores que apuntaban al item
    // (incluido el buyer). Van guardados en preservedLikers para restaurar.
    Object.entries(players).forEach(([p, d]) => {
      if (d.likedObjectId === mObj.uid) updates[`players/${p}/likedObjectId`] = null;
    });
    await update(ref(db, "game"), updates);
  ...
  const createPendingFromDirectSale = async (mObj, buyerPid, sr, dc) => {
    ...
    // Congelar el escrow del buyer (el buyer NO está en deuda en el caso normal —
    // si lo está, la lógica del saleModal ya validó que puede pagar con deuda)
    const updates = {};
    if ((buyer.likes || 0) >= 0) {
      updates[`players/${buyerPid}/likesEscrow`] = (buyer.likesEscrow || 0) + price;
    }
    if (Object.keys(updates).length > 0) {
      await update(ref(db, "game"), updates);
    }
  ...
  const completePendingTransaction = async (txId) => {
    ...
    // Descongelar escrow del buyer (sólo si había)
    if ((buyer.likesEscrow || 0) > 0) {
      updates[`players/${ptx.buyerId}/likesEscrow`] = Math.max(0, (buyer.likesEscrow || 0) - price);
    }
    ...
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", color: th.text, display: "flex", flexDirection: "column" }} onClick={closeAllDropdowns}>
            ...
            <div style={{ padding: "20px 16px", maxWidth: "600px", margin: "0 auto" }}>
              ...
              <div style={{
                ...
                {[
                  ...
                  { id: "pending", label: t.walletPending, count: myPending.length },
      ...
      {feedView && (() => {
        ...
        if (feedView.sourceType === "active") {
          ...
          liveItems = myPendingLive.map((ptx) => ({ ...ptx, _feedKey: `p:${ptx.id}` }));
