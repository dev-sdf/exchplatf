...
export default function Game({ user, onLogout, lang, setLang, theme, setTheme }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: th.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", color: th.text, display: "flex", flexDirection: "column" }} onClick={closeAllDropdowns}>
      ...
      >
        <div style={{ maxWidth: "600px", margin: "0 auto", padding: "10px 14px" }}>
          {/* Fila: avatar + nombre + rating + email + menú */}
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
            ...
            >
              {me.photo ? (
                <img src={me.photo} style={{ width: 40, height: 40, borderRadius: "50%", display: "block" }} />
              ) : (
                <div style={{ width: 40, height: 40, borderRadius: "50%", background: th.bgCard, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 700, color: th.textSecondary }}>
            ...
            <div style={{ minWidth: 0, flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, minWidth: 0 }}>
                <div style={{ fontSize: "14px", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{me.name}</div>
                <button onClick={() => setProfileEditOpen(true)} title={t.profileEditTooltip}
                  style={{ background: "none", border: "none", cursor: "pointer", color: th.textMuted, fontSize: 12, padding: 2, lineHeight: 1, flexShrink: 0 }}
                  aria-label={t.profileEditTooltip}>✎</button>
              </div>
              <div style={{ fontSize: "10px", color: th.textMuted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{getRatingStr(me)}</div>
              {user.email && (
                <div style={{ fontSize: "10px", color: th.textMuted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</div>
              )}
            </div>
            {/* Hamburger menu */}
            <div ref={profileMenuRef} style={{ position: "relative", flexShrink: 0 }} onClick={e => e.stopPropagation()}>
              <button onClick={() => setProfileMenuOpen(o => !o)}
                style={{ background: th.bgCard, border: `1px solid ${th.borderColor}`, borderRadius: "8px", padding: "6px 8px", cursor: "pointer", color: th.text, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1 }}
                aria-label={t.menu}>
              ...
              {profileMenuOpen && (
                <div style={{
                  ...
                  background: th.dropdown, border: `1px solid ${th.dropdownBorder}`,
                  borderRadius: 10, boxShadow: "0 12px 32px rgba(0,0,0,0.4)", zIndex: 120, overflow: "hidden",
                }}>
                  <div onClick={() => { setProfileMenuOpen(false); setActivityModalOpen(true); }}
                    style={{ padding: "12px 14px", fontSize: 13, color: th.text, cursor: "pointer", display: "flex", alignItems: "center", gap: 10, borderBottom: `1px solid ${th.borderLight}` }}>
                    <span style={{ fontSize: 16 }}>📋</span><span>{t.myActivity}</span>
                  </div>
                  <div onClick={() => { setProfileMenuOpen(false); onLogout(); }}
                    style={{ padding: "12px 14px", fontSize: 13, color: "#ef4444", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, fontWeight: 600 }}>
