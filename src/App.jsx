...
import { translations, themes } from "./i18n.js";
  ...
  const controlBar = (
    <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
      <button onClick={toggleTheme} style={{
        background: th.bgInput, border: `1px solid ${th.borderColor}`, borderRadius: "6px",
        padding: "4px 8px", fontSize: "14px", cursor: "pointer", color: th.text, lineHeight: 1,
      }}>{theme === "dark" ? "☀️" : "🌙"}</button>
      {["es", "it", "en"].map(l => (
        <button key={l} onClick={() => setLang(l)} style={{
          background: lang === l ? th.bgHover : "transparent",
          border: lang === l ? `1px solid ${th.borderColor}` : "1px solid transparent",
          borderRadius: "6px", padding: "4px 10px", fontSize: "12px", fontWeight: 600,
          color: lang === l ? th.text : th.textMuted, cursor: "pointer", textTransform: "uppercase",
        }}>{l}</button>
      ))}
    </div>
  );
